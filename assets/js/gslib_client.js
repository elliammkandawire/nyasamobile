...
var LIGHT={
	...
	shadow:" rgba(0,0,0,0.2) 2px 0px 10px 5px;"
}
var PRIMARY={
...
var DARK={
	...
	shadow:" rgba(250,250,250,0.2) 2px 0px 10px 5px;"
}
var TOP=true
var BOTTOM=false
...
var getScale=function(obj){
	return get(obj).style.transition
}
...
function connect_s(object) {
	//secure POST connection
	//object.success : function(output)
	//object.url : link to connect to, object.arguments = arguments ('key'=>value)
	var gs_form= new FormData();//the form data
	//override error method
	object.debug=false;//set debug log off
	object.error=object.error || null
	//create the arguments in a well argumented manner
	for (var key in object.arg) {
 		gs_form.append(key,object.arg[key]);
	}
	var ajax = new XMLHttpRequest();
	ajax.open("POST", object.url, false);
	ajax.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			//all is well, return the outcome here
			object.success(ajax.responseText)
		}
		else{
			if(this.readyState==0){
				object.error("Your request was not initialized")
			}
			if(this.status==403){
				object.error("Access Denied. [Authorisation error]")
			}
			else if(this.status==404){
				object.error("The page "+object.url+" could not be located.")
			}
		}
	};
	ajax.send(gs_form);
...
//new longPress/click listener
var gsduration=0
obj.longClick=function(duration,fn){
	var timer_count
	this.addEventListener("mousedown",function(evt){
		timer_count=setInterval(function(){
			gsduration=gsduration+0.05

			if(gsduration>=(duration/10)){
				//longpresssed
				fn(gsduration)
				gsduration=0
...
function Toast(message,delay,position,theme){
	...
	h3.style.backgroundColor=theme.background || "rgba(17,17,17,0.7)"
	h3.style.color=theme.color || "white"
	h3.style.textAlign="center"
	h3.style.fontSize="14px"
	h3.style.transition="300ms"
	h3.style.zIndex="10000000000"
	h3.style.webkitTransition="300ms"
	h3.style.padding="5px"
	h3.style.paddingTop="10px"
	h3.style.paddingBottom="10px"
	h3.style.border=theme.border || ""
	h3.style.opacity="0"
	h3.style.boxShadow=theme.shadow
